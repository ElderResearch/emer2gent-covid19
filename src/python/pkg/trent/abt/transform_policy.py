...
Binary features indicating if policy was in place at time stamp.
Note if None Issued then policy was not implmented
- travel_limit
- stay_home
- educational_fac
- phase_1
...
def get_policy_ABT(Policy_df, CDC_df):
    ...
    Args:
        ...
        CDC_DF:  for time stamp to construct on
...
def state_policy_abt(state_idx, state_level, Policy_df, CDC_df):
    """ Construct ABT for individual state or county from the policy data
     Args:
        state_idx : index of state in Policy_df
        state_level: True = state data only & False = county exists
        Policy_df: Raw policy data
        CDC_DF:  for time stamp to construct on
    Returns:
    ...
    # STEP2: Get binary features for policies with same prefix on start and end
    ...
    for k in col_names:
        ...
        # Get binary column using function above:
    ...
    # pairs of columns that are associated by start and end date just with different naming conventions
    col_name_pairs = [
    ...
    for i, k in enumerate(col_name_pairs):
        ...
        # Phase_i columns naming conventions
...
def get_binary_col(start_date, end_date, date):
    """ Gets Binary features for a policy specified by start and end date
     Args:
