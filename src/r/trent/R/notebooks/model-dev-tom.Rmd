...
Set `FORCE_MODEL_RUN <- TRUE` to re-run all calculations, even if a cached
...
This helps tremendously, with $R^2 = 0.60(16)$ on held out data.
...
plot_model(s2_pol_cov$object[[1]], type = "re")[[2]]  # state-level
```
...
## Policy with policy-time interactions

- policy and time (policy impact over time)

Still thinking about mobility.

```{r}
s2_pol_cov1_it <- here("data/ts_model_s2_pol_kcov_it.rds") %>%
  cache_operation({
    run_model_vfold(
     formula = target ~ 1 + 
       policy_recoded + 
...
```{r, eval=F}
s2_pol_cov1_it %>% extract_cv_glance()
s2_pol_cov1_it %>% extract_cv_perf()
...
```{r, eval=F}
plot_models(s2_pol_cov1_econ_mob_it$object, dot.size = 2)
plot_model(s2_pol_cov1_econ_mob_it$object[[1]], type = "re")[[2]]  # state-level
```

```{r, eval=F}
diags_ <- plot_model(s2_pol_cov1_econ_mob_it$object[[1]], type = "diag")
resids_ <- plot_model(s2_pol_cov1_econ_mob_it$object[[1]], type = "resid")
print(resids_)
```
...
s2_pol_cov1_econ_mob_it_ex <- s2_pol_cov1_econ_mob_it %>% 
  cv_extract_preds(.per_fold = 2)

plot_w_policy(s2_pol_cov1_econ_mob_it_ex)
```
...
cf. statistical rethinking:

 - post-treatment bias (mobility, economy)
 - masking relationships (gender? ages?)
 - center all the not-centered predictors
 - Gelman rescaling
 
```{r}
s2_pol_simpler <- here("data/ts_model_s2_pol_simpler.rds") %>%
  cache_operation({
    run_model_vfold(
     formula = target ~ 1 + 
       policy_recoded + 
...
```{r}
s2_pol_simpler %>% extract_cv_glance()
s2_pol_simpler %>% 
  extract_cv_tidy() %>% 
  print(n = Inf)
```
...
```{r}
s2_pol_simpler_it <- here("data/ts_model_s2_pol_simpler_it.rds") %>%
  cache_operation({
    run_model_vfold(
     formula = target ~ 1 + 
       policy_recoded + 
       time_days_since_inf1 + 
       acs_pop_total:time_days_since_inf1 + 
...
```{r}
s2_pol_simpler_it %>% extract_cv_glance()
s2_pol_simpler_it %>% 
  extract_cv_tidy() %>% 
  print(n = Inf)
```
...
```{r}
s2_pol_simpler_rt <- here("data/ts_model_s2_pol_simpler_rt.rds") %>%
  cache_operation({
    run_model_vfold(
     formula = target ~ 1 + 
       policy_recoded + 
...
```{r}
s2_pol_simpler_rt %>% extract_cv_glance()
s2_pol_simpler_rt %>% 
  extract_cv_tidy() %>% 
  print(n = Inf)
```
...
```{r}
s2_pol_simpler_rt_complex <- here("data/ts_model_s2_pol_simpler_rt_complex.rds") %>%
  cache_operation({
    run_model_vfold(
     formula = target ~ 1 + 
       policy_recoded + 
       ...
       cov_total_tests + cov_pos_tests_frac + 
       policy_recoded:time_days_since_inf1 + 
       (1 + time_days_since_inf1 + policy_recoded:time_days_since_inf1 | state_code/county_fip),
     data = sample2_abt
    )
  })
```
...
```{r}
s2_pol_simpler_rt_complex %>% extract_cv_glance()
s2_pol_simpler_rt_complex %>% 
  extract_cv_tidy() %>% 
  print(n = Inf)
```
...
### Stats tables
 ...
 - The CCC and R-squared values either went down or became very uncertain.
...
## Policy with mobility

The econ was good enough to keep.
...
```{r}
s2_pol_cov1_econ_mob %>% extract_cv_glance()
...
```{r, eval=F}
s2_pol_cov1_econ_mob %>% extract_cv_glance()
s2_pol_cov1_econ_mob %>% extract_cv_perf()
...
### Fit charts
 ...
 - Results are consistent
   - 85+ needs to go
 - Still confusing correlations with causation, esp. with mobility
   (add imputation flag?)
 - By including mobility, which is heavily influenced by policy, 
   the policy signals are disappearing.

```{r, eval=F}
plot_models(s2_pol_cov1_econ_mob$object, dot.size = 2)
plot_model(s2_pol_cov1_econ_mob$object[[1]], type = "re")[[2]]  # state-level
```
...
s2_pol_cov1_econ_mob_ex <- s2_pol_cov1_econ_mob %>% cv_extract_preds(.per_fold = 2)
plot_w_policy(s2_pol_cov1_econ_mob_ex)
```
