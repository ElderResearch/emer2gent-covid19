...
These will be corrected by replacing the errant value with the last known
value.
```{r}
d %>% 
  arrange(state, county, date) %>% 
...
Confirm the total number of confirmed covid cases in the US (at least, according
to the data we've collected). As of June 6 we should be near the 2million mark
```{r}
d %>% 
  group_by(state, county) %>% 
...
policies <- select(dd, state, county, county_fip, all_of(policy_cols)) %>% 
  ...
  imap_dfc(~ if (.y %in% policy_cols) ifelse(.x == '1', .y, .x) else .x) %>% 
...
filter(dd, county_fip %in% ex_counties) %>% 
  ...
  geom_rect(
    ...
    aes(xmin=start, xmax=stop, ymin=-Inf, ymax=Inf, fill=policy), alpha=0.25, 
    inherit.aes=FALSE
  )
...
categories with the exception of income. Hence, we will focus on
race and age, taking the proportion of each group within a county.  
...
dd <- dd %>% 
  ...
  rowwise() %>%
  mutate(
    acs_age_05_17 = sum(c_across(acs_age_05_09:acs_age_15_17)), 
    acs_age_18_29 = sum(c_across(acs_age_18_19:acs_age_25_29)), 
    acs_age_30_49 = sum(c_across(acs_age_30_34:acs_age_45_49)), 
    acs_age_50_64 = sum(c_across(acs_age_50_54:acs_age_62_64)), 
    acs_age_gt_65 = sum(c_across(acs_age_65_66:acs_age_85_up)), 
    ) %>%
  ungroup()

demo_cols <- c('acs_age_lt_05', 'acs_age_05_17', 'acs_age_18_29',
               'acs_age_30_49', 'acs_age_50_64', 'acs_age_gt_65')
