...
Then, backsmooth in the same way as other cumulative counts.
Note, though, that the dates for total and positive tests don't line up,
so we can't reliably compute the _weekly_ percent infection.
...
dd <- dd %>% 
  ...
  mutate(across(all_of(cov_cols), correct_negative_it))

stopifnot(all(dd$cov_pos_tests >= 0))
stopifnot(all(dd$cov_total_tests >= 0))

# Also compute week-over-week changes like the target
dd <- dd %>% 
  group_by(state, county) %>% 
  arrange(state, county, date) %>% 
  mutate(
    cov_tests_pos_frac = cov_pos_tests / (cov_total_tests + 1e-7),
    cov_tests_tot_week = cov_total_tests - lag(cov_total_tests, n = 7),
    cov_tests_pos_frac_week = cov_tests_pos_frac - lag(cov_tests_pos_frac, n = 7)
  ) %>% 
  ungroup()

stopifnot(min(dd$cov_tests_pos_frac, na.rm = T) >= 0)
stopifnot(max(dd$cov_tests_pos_frac, na.rm = T) <= 1)

stopifnot(min(dd$cov_tests_tot_week, na.rm = T) >= 0)
stopifnot(min(dd$cov_tests_pos_week, na.rm = T) >= 0)

stopifnot(min(dd$cov_tests_pos_frac_week, na.rm = T) >= -1)
stopifnot(max(dd$cov_tests_pos_frac_week, na.rm = T) <= 1)
```

```{r}
# Remove the NAs induced by our choice of target
dd <- dd %>% filter(!is.na(confirmed_cum_lag_7))

stopifnot(all(!is.na(dd$target)))
stopifnot(all(!is.na(dd$cov_tests_tot_week)))
stopifnot(all(!is.na(dd$cov_tests_pos_frac_week)))
```
