...
**NOTE: Report text has not been updated to match calculations.**
...
abt <- abt %>% 
  mutate(
    ...
    across(c(where(is.numeric), -target, -county_fip), ~ ., .names = "{col}_sc"),
    # Transform select columns
    across(c(matches("age.*_sc"), female_sc, income_sc, minority_sc), log),
    across(c(density_sc, pop_sc), log),
    cov_tests_tot_week_sc = sqrt(cov_tests_tot_week_sc),
