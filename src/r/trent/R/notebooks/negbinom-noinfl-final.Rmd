...
ftrs <- tibble::tribble(
  ...
  "temp",              "Covariate",   "Daily mean temperature" ,
  "humid",             "Covariate",   "Daily mean relative humidity" ,
  "density",           "Covariate",   "County population density" ,
  "pop",               "Covariate",   "County population" ,
...
We normalize our data by applying $\log$ and $\log(1+x)$ transformations to highly skewed inputs, including county-level Census proportions. We also apply a [quantile transformation][qn] to both of our time-related inputs. Following [Gelman][gelman-08], we standardize our continuous inputs by centering and then scaling by two standard deviations.
...
The all-relevant feature test does not highlight issues with collinearity, so we also examine our data for highly correlated features. The table below lists features with correlation coefficients $|r|\ge 0.7$, a heuristic threshold for excluding inputs from the model.
...
The overdispersion parameter $r$ is similarly stable; averaged across folds, it takes the value $r = `r rvals_str`$. We also provide the coefficients themselves in the table below, aggregated across folds:
...
model %>% 
  ...
  transmute(
    ...
    `Fitted Value` = sprintf("$%.03f(%.03f)$", estimate, std.error)
  ) %>% 
  arrange(Term) %>% 
  knitr::kable(align = c("l", "r"))
```

Finally, we plot per-state random effects for one of our cross-validation folds, with $\pm 2 \sigma$ confidence intervals.
...
We see from the above coefficient tables that policy effects contribute over time to the (local) progression or suppression of the COVID-19 epidemic in a complex way. Stay-at-home orders, for example, are associated with higher rates of infection on average, perhaps because they are enacted in response to quickly growing case rates, but their negative interaction with time shows they are simultaneously associated with a lessening of this rate over time. Note also that the coefficient `time_sc`, the time since the first measured infection in a county, is negative as well.

```{r, echo=F, fig.width=6}
model_int2_plot + theme(text = element_text(size = 8))
```

The nonlinear transformations of time variables further complicate real understanding of these results from tables, so we demonstrate the effect below by sampling from the fitted model coefficients for the intercept, policy, and time since a policy was enacted, holding other predictors at their average values (i.e., zero):
...
\end{aligned}
$$
Draws for the coefficients are taken from normal distributions parameterized by each coefficient's fitted parameters. We see that, as expected, stay-at-home orders are associated with decreasing case rates; no policy being issued is associated with a growth in cases. The effects of phased reopening are less clear, especially any difference in real effect among them.

```{r sim1, echo=F, fig.width=5}
...
samples %>% 
  ...
  labs(x = "Time in phase (days)", y = "Growth in infection count") + 
  theme(text = element_text(size = 8))
```

Or, if we applied the same to a county with a higher average number of infections (note the differing scales on the $y$-axis):

```{r sim2, echo=F, fig.width=5}
# Random draws
samples <- map_dfr(1:50, ~ {
  times %>% 
    rowwise() %>% 
